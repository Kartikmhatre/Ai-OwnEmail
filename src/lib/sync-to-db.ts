...
async function upsertEmail(email: EmailMessage, index: number, accountId: string) {
    ...
    try {
        ...
        // 3. Upsert Email (MongoDB version - using embedded from and ID arrays)
        await db.email.upsert({
            ...
            update: {
                ...
                from: { name: email.from.name, address: email.from.address, raw: email.from.raw },
                toIds: toAddresses.map(a => a!.id),
                ccIds: ccAddresses.map(a => a!.id),
                bccIds: bccAddresses.map(a => a!.id),
                replyToIds: replyToAddresses.map(a => a!.id),
