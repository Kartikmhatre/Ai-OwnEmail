...
export const mailRouter = createTRPCRouter({
    ...
    getReplyDetails: protectedProcedure.input(z.object({
        ...
        // Fetch email addresses for to, cc, bcc IDs
        const toAddresses = await ctx.db.emailAddress.findMany({
            where: { id: { in: lastExternalEmail.toIds } },
            select: { id: true, name: true, address: true }
        });
        const ccAddresses = await ctx.db.emailAddress.findMany({
            where: { id: { in: lastExternalEmail.ccIds } },
            select: { id: true, name: true, address: true }
        });

        if (input.replyType === 'reply') {
            return {
                to: [{ name: lastExternalEmail.from.name, address: lastExternalEmail.from.address }],
            ...
            return {
                to: [
                    { name: lastExternalEmail.from.name, address: lastExternalEmail.from.address },
                    ...toAddresses.filter(addr => addr.address !== account.emailAddress)
                ],
                cc: ccAddresses.filter(addr => addr.address !== account.emailAddress),
                from: { name: account.name, address: account.emailAddress },
                subject: `${lastExternalEmail.subject}`,
                id: lastExternalEmail.internetMessageId
            };
